    ...
    <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background-color: #ffffff; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
        <tr>
            <td style="background-color: #4a90e2; text-align: center;">
                <p style="font-size: 54px; line-height: 54px; font-weight: 800;">Subly</p>
            </td>
        </tr>
        <tr>
            <td style="padding: 40px 30px;">
                ...
                <p style="font-size: 16px; margin-bottom: 25px;">Your <strong>${subscriptionName}</strong> subscription is set to renew on <strong style="color: #4a90e2;">${renewalDate}</strong> (${daysLeft} ${daysLeft === 1 ? "day" : "days"} from today).</p>
        ...
        <tr>
            <td style="background-color: #f0f7ff; padding: 20px; text-align: center; font-size: 14px;">
                <p style="margin: 0 0 10px;">
                    Subly Inc. | 123 Main St, Anytown, AN 12345
                </p>
                <p style="margin: 0;">
...
type ReminderInput = Omit<ReminderEmailData, "daysLeft">;

export const emailTemplates = [
  {
    label: "7 days before reminder",
    generateSubject: (data: ReminderInput) =>
      `Reminder: Your ${data.subscriptionName} subscription renews in 7 days!`,
    generateBody: (data: ReminderInput) =>
